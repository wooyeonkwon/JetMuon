...
CONDITIONS = [
    {
        "Jet_muEF": {"condition": lambda i: "Jet_muEF[i] > -1", "ymean": "Jet_muEF", "color": ROOT.kBlack, "marker": 4},
...
BRANCHES = [ # see details at Profile1D_def in the module interface/root_tool.py
    {"output": "allJet", "name": "Jet_eta", "ymean" : None, "title" : "", "bins": 60, "xmin": -3.0, "xmax": 3.0, "xtitle": "muonEta", "ytitle": "Multiplicity", "condition": None},
    {"output": "muonJet", "name": "Jet_eta", "ymean" : None, "title" : "", "bins": 60, "xmin": -3.0, "xmax": 3.0, "xtitle": "muonEta", "ytitle": "Multiplicity", "condition": None},
]
...
def draw_profile(root_rdf, tree_name, branch, condition, output_dir="."):
    profiles = {}

    # make Profile for all CONDITIONS
    for name, style in condition.items():
        cond = style["condition"]
        y = style["ymean"]
        branch["condition"] = cond # set condition
        branch["ymean"] = y
        profiles[name] = Profile1D_filtery_def(root_rdf, branch)
    ...
    with canvas_lock:
        ...
        for idx, (name, style) in enumerate(condition.items()):
            profile = profiles[name].GetPtr()
    ...
    for tree_name in TREE_NAMES:
        ...
        for branch in BRANCHES:
            ...
            try:
                ...
                draw_profile(root_rdf, tree_name, branch, condition, output_dir)
